...
```bash
git rm --cached <file>        # 只把它从 Git 的跟踪名单里删掉，本地文件原样保留
git commit -m "停止跟踪 xxx"    # 「不再跟踪」本身也是一次改动，要提交才生效
```
...
# 下面这次提交只落在「试验」分支上，main 那条线完全不受影响
echo "试验内容" > 试验.md
git add 试验.md
...
```bash
git switch main                # 合并前先切到「接收方」，是把别人并进当前分支
git merge 试验                 # 把试验分支上的提交接到 main 上
ls                             # 现在 main 上也有试验.md 了
git branch -d 试验              # 删掉已合并的分支（-d 是 delete）；没合并过它会拒绝删，防误删
```
...
# 回主线合并：两边都改了同一个文件的同一行，Git 无法自动取舍，必然冲突
git switch main
git merge 分支A
```
...
git commit -m "说明"    # 存档；说明写清楚这次改了什么，以后翻历史全靠它
git push                # 推到 GitHub
```
...
- [ ] 你的学习仓库有 5 个以上语义清晰的 commit
- [ ] 完整走过一次：建分支 → 改动 → 提交 → 合并 → 删分支
- [ ] **亲手制造并解决过一次 merge 冲突**
- [ ] 用过 `git restore` 丢弃改动、`git restore --staged` 退出暂存区
- [ ] 成功 push 到 GitHub
...
# 撤销
git restore <file>            # 丢弃工作区改动（不可逆）
git restore --staged <file>   # 退出暂存区，改动仍留在工作区
...
git switch main               # 切换
git branch                    # 列出
git merge <branch>            # 把它合并进当前分支
git branch -d <branch>        # 删除（未合并会拒绝）
git stash                     # 临时收起改动
git stash pop                 # 取回
```
...
U0.2 教的是「一个人线性地写代码」够用的 Git。**但你的实际工作方式是并行的**——同时开几个 agent 干不同的活。

那种场景要用到这些，都在 [U0.12 · Git 分支与 worktree 实战](U0.12-Git分支与worktree实战.md)：

| 命令 | 解决什么 |
|---|---|
| `git reflog` | **误操作的兜底**。`reset --hard` 丢掉的提交、删错的分支，都能找回来 |
| `git worktree` | **多个目录同时在不同分支上**，三个 agent 并行互不干扰 |
| `git rebase -i` | 把 agent 产生的一串碎提交整理成有意义的两三个 |
| `git cherry-pick` | 从一个整体放弃的分支里，捞出局部有价值的提交 |
| `git bisect` | 20 个提交里二分定位是哪次改坏的 |
| `git fetch` vs `pull` | 先看远程改了什么再决定合不合 |
| `--force-with-lease` | 安全的强推 |

**先把这个单元的基础练熟，U0.12 建立在它之上。**
