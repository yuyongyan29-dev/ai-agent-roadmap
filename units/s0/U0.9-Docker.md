...
实现靠 Linux 内核的两个特性：
  ...
  cgroups   → 限制能用多少 CPU、内存

所以容器本质上就是一个被隔离和限制了的普通进程，不是一台虚拟的电脑。
```
...
```dockerfile
# slim 版只带跑 Python 必需的系统库，比完整版小几百 MB
FROM python:3.13-slim

# 直接从 uv 官方镜像里拷可执行文件，比 pip install uv 快，也不会混进项目依赖
COPY --from=ghcr.io/astral-sh/uv:latest /uv /usr/local/bin/uv

WORKDIR /app

# 先只拷依赖声明：只要这两个文件没变，这一层就命中缓存，不用重装依赖
# --frozen 强制按 uv.lock 装，保证镜像里的版本和本地一模一样
COPY pyproject.toml uv.lock ./
RUN uv sync --frozen --no-install-project

# 代码放在依赖之后拷，改代码时只从这一层开始重建
COPY src/ ./src/
RUN uv sync --frozen

# EXPOSE 只是写给人和工具看的声明，真正开放端口靠 docker run -p
EXPOSE 8000
# 必须监听 0.0.0.0：只听 127.0.0.1 的话，容器外面连不进来
...
```
# 本机虚拟环境，镜像里会按 lock 文件重装，拷进去只会白白撑大体积
.venv/
# 版本历史对运行没有用，仓库大了还会拖慢构建
.git/
__pycache__/
*.pyc
.pytest_cache/
...
services:
  db:
    image: postgres:17
    environment:
      # postgres 镜像第一次启动时会用这三个变量建库建用户
      POSTGRES_USER: app
      POSTGRES_PASSWORD: dev
      POSTGRES_DB: csvstats
    volumes:
      # 命名卷：容器删掉数据还在，不写这行 down 一次数据就没了
      - pgdata:/var/lib/postgresql/data
    ports:
      - "5432:5432"
    healthcheck:
      # 进程起来不等于能接受连接，pg_isready 探到就绪才算健康
  ...
  api:
    build: .
    environment:
      # 主机名写服务名 db，不能写 localhost —— 容器里的 localhost 指容器自己
      DATABASE_URL: postgresql+psycopg://app:dev@db:5432/csvstats
    ports:
      - "8000:8000"
    depends_on:
      db:
        # 等 db 的 healthcheck 通过再启动，否则 api 一上来就连不上数据库崩掉
        condition: service_healthy
    volumes:
      - ./src:/app/src          # 开发时挂载代码，改了立刻生效

volumes:
  # 声明上面用到的命名卷，交给 Docker 管理存储位置
  pgdata:
```
...
- [ ] `docker compose up` 一条命令同时起 API 和数据库
- [ ] 浏览器能访问 `http://localhost:8000/docs` 并调通接口
- [ ] 数据写进数据库，`docker compose down` 再 `up` 之后数据还在
- [ ] `docker compose down -v` 之后数据被清空（验证卷确实在起作用）
- [ ] 写了 `.dockerignore`，`.env` 和 `.venv` 都在里面
- [ ] 镜像体积在 500MB 以内（`docker images` 查看）
